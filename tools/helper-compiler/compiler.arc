    ...
    cons (reccase
           ...
           (with (var-vals body)
             (with (vars (map car  (pair var-vals))
               ...
               (union is
                      (dedup (flat (map [find-free _ b] vals)))
                      (find-free body (union is vars b)))))

           (do body
               (dedup (flat (map [find-free _ b] body))))

           (%if test-then-else
                (dedup (flat (map [find-free _ b] test-then-else))))
           ...
           (dedup (flat (map [find-free _ b] x)))
    ...
    cons (reccase
           ...
           (with (var-vals body)
             (with (vars (map car  (pair var-vals))
                         vals (map cadr (pair var-vals)))
               (union is
                      (dedup (flat (map [find-sets _ v] vals)))
                      (find-sets body (set-minus v vars)))))

           (do body
               (dedup (flat (map [find-sets _ v] body))))

           (%if test-then-else
                (dedup (flat (map [find-sets _ v] test-then-else))))
